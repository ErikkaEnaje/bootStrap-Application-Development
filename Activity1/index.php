<html>
   <head>
        <title>3BSCS-2||ENAJE||ACT1</title>
         <link rel="stylesheet" href="css/bootstrap.min.css">
         <script src="js/jquery.js"></script>
         <scrpit src="js/bootstrap.js"></script>
	
   </head>
   <body>
    <h1>ACTIVITY 1</h1>
    <div class="container-fluid">
	<table class="table">
	      <tr class="danger">
		      <th>ID</th>
			  <th>Name</th>
			  <th>Remarks</th>  
		  </tr>
		  <tr>
		      <td>001</td>
			  <td>Alex</td>
			  <td>Passed</td>  
		  </tr>
		  <tr>
		      <td>002</td>
			  <td>Cris</td>
			  <td>Passed</td>  
		  </tr>
		  <tr>
		      <td>003</td>
			  <td>Jack</td>
			  <td class="danger">Failed</td>  
		  </tr>
	
	</table>
	
	
	<table class="table table-striped">
	      <tr class="info">
		      <th>ID</th>
			  <th>Name</th>
			  <th>Remarks</th>  
		  </tr>
		  <tr>
		      <td>001</td>
			  <td>Alex</td>
			  <td>Passed</td>  
		  </tr>
		  <tr>
		      <td>002</td>
			  <td>Cris</td>
			  <td>Passed</td>  
		  </tr>
		  <tr>
		      <td>003</td>
			  <td>Jack</td>
			  <td class="danger">Failed</td>  
		  </tr>
	
	</table>
	
	
	<table class="table table-condensed">
	      <tr class="warning">
		      <th>ID</th>
			  <th>Name</th>
			  <th>Remarks</th>  
		  </tr>
		  <tr>
		      <td>001</td>
			  <td>Alex</td>
			  <td>Passed</td>  
		  </tr>
		  <tr>
		      <td>002</td>
			  <td>Cris</td>
			  <td>Passed</td>  
		  </tr>
		  <tr>
		      <td>003</td>
			  <td>Jack</td>
			  <td class="danger">Failed</td>  
		  </tr>
	
	</table>
	
	
	<table class="table table-hover table-bordered">
	      <tr class="success">
		      <th>ID</th>
			  <th>Name</th>
			  <th>Remarks</th>  
		  </tr>
		  <tr>
		      <td>001</td>
			  <td>Alex</td>
			  <td>Passed</td>  
		  </tr>
		  <tr>
		      <td>002</td>
			  <td>Cris</td>
			  <td>Passed</td>  
		  </tr>
		  <tr>
		      <td>003</td>
			  <td>Jack</td>
			  <td class="danger">Failed</td>  
		  </tr>
	
	</table>
	
	
	<table class="table table-bordered">
	      <tr class="text-primary bg-primary">
		      <th>ID</th>
			  <th>Name</th>
			  <th>Remarks</th>  
		  </tr>
		  <tr>
		      <td>001</td>
			  <td>Alex</td>
			  <td>Passed</td>  
		  </tr>
		  <tr>
		      <td>002</td>
			  <td>Cris</td>
			  <td>Passed</td>  
		  </tr>
		  <tr>
		      <td>003</td>
			  <td>Jack</td>
			  <td class="danger">Failed</td>  
		  </tr>
	
	</table>
	
	
	</div>
	<hr/>
	<br/>
	
	<div class="container">
	   <p class="text-success bg-success">Success</p>
	   <p class="text-danger bg-danger">Danger</p>
	   <p class="text-info bg-info">Info</p>
	   <p class="text-primary bg-primary">Primary</p>
	   <p class="text-muted">Muted</p>
	   <p class="text-warning bg-warning">Warning</p>
	   
	   <p class="lead">Information Technology</p>
	   <p class="small">Information Technology</p>
	   <p class="text-center">Information Technology</p>
	   <p class="text-right">Information Technology</p>
	   <p class="text-uppercase">Information Technology</p>
	   <p class="text-lowercase">Information Technology</p>
	  
	</div>
   
    <hr/>
    <br/>
    
	
	<div class="container-fluid">
	  <div class="row">
	   <div class="col-md-1" style="background: yellow">grid</div>
	   <div class="col-md-1" style="background: pink">grid</div>
	   <div class="col-md-5" style="background: blue">grid</div>
	  </div> 
	  
	   <div class="row">
	     <div class="col-md-6" style="background: orange">grid</div>
	   
	   </div>
	</div>
   
    <hr/>
    <br/>
   

   <div class="container-fluid">
       <header class="Jumbotron">
	       <h1>Container fluid</h1>
	   </header>
   
    </div>
   
    <hr/>
	<br/>
   
   
   <div class="container">
       <header class="Jumbotron">
	       <h1>Container</h1>
	   </header>
   
    </div>
   
   
    <hr/>
	<br/>
   
   
   <div class="Jumbotron">
	      <h1>Hello</h1>
	  
	 </div>
  
    
   </body>
</html>